...
public class DatabaseGenerator {
    ...
    @Transactional
    public void generateData(int quantity, boolean nearby, City city) {
        csvReader = new CSVReader();
        this.listOfNames = csvReader.read(new File("src/main/resources/otherData/namesPl.csv"));
    ...
    private void prepareLoginAndPassword() {
      ...
      int counter = 0;
        boolean stmnt = true;
        do {
            User exist = userRepository.getUserByUserName(data);
            if(exist == null) {
                user.setUsername(data);
                user.setPassword(new BCryptPasswordEncoder().encode(data));
                stmnt = false;
            } else {
                counter++;
                data += counter;
